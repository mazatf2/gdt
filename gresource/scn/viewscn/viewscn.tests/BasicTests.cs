...
using System.Text;
using gdt.gresource.scn.viewscn;
using Godot;
using twodog.fixture;
using twodog.xunit;

namespace viewscn.Tests;
...
public class BasicTests(GodotHeadlessFixture godot) {
	...
	public void ParseHeader() {
		using var stream = File.Open("viewscn.tests/data/scene2.scn.nocompression", FileMode.Open);
		using var reader = new BinaryReader(stream, encoding: Encoding.UTF8, leaveOpen: false );
		var v = new ViewScn();
		v.Parse(reader);
		Debugger.Break();
	}
